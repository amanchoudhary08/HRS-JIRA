...
const STATUS_CFG: Record<string, { cls: string; dot?: string; pulse?: boolean }> = {
  DEPLOYED:          { cls: 'bg-emerald-500/10 text-emerald-300 border-emerald-500/20', dot: 'bg-emerald-400' },
  ...
  AWAITING_APPROVAL: { cls: 'bg-amber-500/10 text-amber-300 border-amber-500/20' },
  RUNNING:           { cls: 'bg-arc/10 text-arc-400 border-arc/20', dot: 'bg-arc', pulse: true },
...
export function StatusBadge({ status, size = 'md' }: { status: string; size?: 'sm' | 'md' }) {
  const cfg = STATUS_CFG[status] ?? { cls: 'bg-slate-500/10 text-slate-400 border-slate-500/15' }
  const pad = size === 'sm' ? 'px-1.5 py-0.5 text-[10px] gap-1' : 'px-2 py-1 text-xs gap-1.5'
  return (
    <span className={clsx('inline-flex items-center rounded border font-medium font-mono tracking-wide', pad, cfg.cls)}>
      {cfg.dot && (
        <span className="relative flex h-1.5 w-1.5 flex-shrink-0">
          {cfg.pulse && <span className={clsx('animate-ping absolute inline-flex h-full w-full rounded-full opacity-60', cfg.dot)} />}
...
export function KPICard({
  title, value, unit, trend, icon: Icon, color = '#22d3ee',
}: {
  title: string; value: string | number; unit?: string
  trend?: 'up' | 'down' | 'flat'; icon: React.ElementType; color?: string
}) {
  return (
    <motion.div
      ...
      className="relative rounded-xl p-5 overflow-hidden transition-all duration-200 group"
      style={{ background: 'rgba(8,14,28,0.9)', border: '1px solid rgba(255,255,255,0.06)' }}
    >
      ...
      />
      {/* accent corner glow */}
      <div className="absolute top-0 right-0 w-32 h-32 opacity-[0.06] pointer-events-none rounded-full blur-2xl"
      ...
      <p className="text-3xl font-black tracking-tight" style={{ color: '#e8f0fe', fontVariantNumeric: 'tabular-nums' }}>
        {value}
      ...
      {trend && (
        <div className={clsx(
          ...
          {trend === 'up' ? 'Above target' : trend === 'down' ? 'Below target' : 'On target'}
        </div>
      )}
    </motion.div>
  )
...
export function Avatar({ initials, color, size = 'md' }: { initials: string; color: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={clsx('rounded-lg flex items-center justify-center font-black flex-shrink-0', sz)}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      {action && (
        ...
        >
          <button onClick={action.onClick} className="btn-arc text-xs px-4 py-2">
            {action.label}
          </button>
        </motion.div>
      )}
...
export function Card({ children, className, hover = false }: {
  ...
  return (
    <div className={clsx('panel', hover && 'hover:border-arc/20 hover:bg-surface-2 transition-all duration-150 cursor-pointer', className)}>
      {children}
    </div>
  )
}
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className={clsx('relative z-10 w-full rounded-xl shadow-panel', maxW)}
            style={{ background: '#0d1628', border: '1px solid rgba(34,211,238,0.15)' }}
          >
...
export function Button({
  ...
  size?: 'sm' | 'md' | 'lg'; className?: string; icon?: React.ElementType
  loading?: boolean; disabled?: boolean; onClick?: () => void; type?: 'button' | 'submit' | 'reset'
}) {
  const variantCls = {
    primary: 'btn-arc',
    secondary: 'btn-dim',
  ...
  const sizeCls = size === 'sm' ? 'text-xs px-3 py-1.5' : size === 'lg' ? 'text-sm px-5 py-2.5' : 'text-sm px-4 py-2'
  return (
    <motion.button
      ...
      {loading ? <Spinner size="sm" /> : Icon ? <Icon size={size === 'sm' ? 12 : 14} className="flex-shrink-0" /> : null}
      {children}
    </motion.button>
  )
}
...
export function Input({ label, error, className, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label?: string; error?: string }) {
  return (
    <div className="space-y-1.5">
      {label && <label className="text-xs font-semibold text-slate-400 block uppercase tracking-wide">{label}</label>}
      <input {...props} className={clsx('field', error && '!border-red-500/40', className)} />
...
export function Select({ label, error, className, children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label?: string; error?: string }) {
  return (
    <div className="space-y-1.5">
      {label && <label className="text-xs font-semibold text-slate-400 block uppercase tracking-wide">{label}</label>}
      <select {...props} className={clsx('field', error && '!border-red-500/40', className)}>{children}</select>
      {error && <p className="text-xs text-red-400">{error}</p>}
    </div>
  )
}
...
export function Textarea({ label, error, className, ...props }: React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label?: string; error?: string }) {
  return (
    <div className="space-y-1.5">
      {label && <label className="text-xs font-semibold text-slate-400 block uppercase tracking-wide">{label}</label>}
      <textarea {...props} className={clsx('field resize-none', error && '!border-red-500/40', className)} />
      {error && <p className="text-xs text-red-400">{error}</p>}
    </div>
  )
}
      ...
      <AnimatePresence>
        {show && (
          <motion.div
            ...
            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1.5 rounded-lg text-xs text-slate-200 whitespace-nowrap shadow-xl z-50 pointer-events-none"
            style={{ background: '#131f36', border: '1px solid rgba(34,211,238,0.15)' }}
          >
            {label}
          </motion.div>
        )}
