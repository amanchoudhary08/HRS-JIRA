...
import type { SkillFile, Persona, SkillStatus } from "../../types/ai";
...
export default function SkillsPage() {
  ...
  const { data: skills, isLoading } = useQuery<SkillFile[]>({
    queryKey: ["skills"],
    queryFn: () => aiApi.get("/skills").then((r) => r.data),
  });
  ...
  async function deploy(id: number) {
    setDeploying(id);
    try {
      await aiApi.post(`/skills/${id}/deploy`);
  ...
  const all = skills ?? [];
  const counts = all.reduce<Record<string, number>>((a, s) => {
  ...
  return (
    <div>
      <PageHeader
        title="Skill Files"
        description="AI-generated role specifications for every persona"
        backTo="/projects"
      />
        ...
        {TABS.map((t) => (
          ...
          >
            ...
            {t.value !== "ALL" && counts[t.value] ? (
              <span className="ml-1 opacity-60">({counts[t.value]})</span>
            ) : null}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Generate a skill file from a persona to get started."
        />
      ) : (
        <div
            ...
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                {[
                  ...
                  <th
                    key={h}
                    className={`text-left text-[10px] font-bold uppercase tracking-[0.1em] text-slate-600 px-4 py-3 ${!h && "text-right"} ${h === "Department" && "hidden md:table-cell"} ${h === "Version" && "hidden sm:table-cell"} ${h === "Updated" && "hidden lg:table-cell"}`}
                  >
                    {h}
                  </th>
                ))}
              ...
              {filtered.map((skill) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2.5">
                        {persona && (
                          <div
                            ...
                            style={{
                              background: `${persona.avatar_color}14`,
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell text-xs font-mono text-slate-500">
                      v{skill.version}
                    </td>
