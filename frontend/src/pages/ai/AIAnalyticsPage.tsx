...
import { Spinner, KPICard, PageHeader } from "../../components/ai/ui";
import type { AnalyticsDashboard } from "../../types/ai";
...
export default function AIAnalyticsPage() {
  const { data: analytics, isLoading } = useQuery<AnalyticsDashboard>({
    ...
    queryFn: () => aiApi.get("/analytics/dashboard").then((r) => r.data),
  });
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <KPICard
          ...
          icon={Bot}
          color="#10b981"
          trend={analytics.active_agents > 0 ? "up" : "flat"}
        />
        <KPICard
          title="Executions Today"
          value={analytics.executions_today}
          icon={Activity}
          color="#6366f1"
        />
        <KPICard
          title="Skill Deployed"
          value={`${analytics.skill_deployed_pct?.toFixed(0) ?? 0}%`}
          icon={FileText}
          color="#f59e0b"
          trend={analytics.skill_deployed_pct >= 50 ? "up" : "down"}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {chartCard(
          ...
          analytics.executions_by_day?.length ? (
        ...
        {chartCard(
          "Skill Files by Department",
          analytics.skill_by_dept?.length ? (
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={analytics.skill_by_dept} barSize={16}>
                ...
                <Legend
                  formatter={(v) => (
                    <span style={{ color: "#8dadd8", fontSize: 10 }}>{v}</span>
                  )}
                />
                <Bar
                  dataKey="deployed"
        ...
        {chartCard(
          ...
          analytics.execution_status_dist?.length ? (
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie
                  data={analytics.execution_status_dist}
                  dataKey="count"
                  nameKey="status"
                  cx="50%"
                  cy="45%"
                  ...
                  innerRadius={32}
                >
                  {analytics.execution_status_dist.map((_, i) => (
                ...
                <Legend
                  formatter={(v) => (
                    <span style={{ color: "#8dadd8", fontSize: 10 }}>
