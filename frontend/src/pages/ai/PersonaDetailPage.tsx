...
import { Spinner, Avatar, StatusBadge, Modal } from "../../components/ai/ui";
import type { Persona, SkillFile, Agent } from "../../types/ai";
...
export default function PersonaDetailPage() {
  ...
  const { data: persona, isLoading } = useQuery<Persona>({
    ...
    queryFn: () => aiApi.get(`/personas/${slug}`).then((r) => r.data),
  });
  const { data: skillFile, isLoading: ls } = useQuery<SkillFile | null>({
    ...
    queryFn: async () => {
      ...
      try {
        return (await aiApi.get(`/skills/${persona.id}`)).data;
      } catch {
        return null;
      }
    },
  ...
  const { data: agent } = useQuery<Agent | null>({
    ...
    queryFn: async () => {
      if (!persona?.id) return null;
      const agents: Agent[] = (await aiApi.get("/agents")).data;
  ...
  async function saveSkill() {
    ...
    setSaving(true);
    try {
      await aiApi.patch(`/skills/${skillFile.id}`, { content: editContent });
  ...
  async function transition(action: string) {
    ...
    try {
      await aiApi.post(`/skills/${skillFile.id}/${action}`);
      qc.invalidateQueries({ queryKey: ["skill-file", persona?.id] });
  ...
  async function runTask() {
    if (!agent || !taskName.trim()) return;
    setRunning(true);
    try {
      await aiApi.post(`/agents/${agent.id}/execute`, {
        task_name: taskName,
  ...
  if (!persona)
    return (
      <div className="text-center py-20 text-slate-500">Persona not found.</div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {persona.data_access?.length > 0 && (
            ...
            >
              <p className="section-title flex items-center gap-1.5">
                <Database size={10} style={{ color: "#22d3ee" }} />
                Data Access
              </p>
          ...
          >
            ...
            {ls ? (
              ...
              <div
                ...
                style={{ border: "1px dashed rgba(34,211,238,0.15)" }}
              >
                <Wand2
                  size={20}
                ...
                <button
                  onClick={() => navigate(`/ai/skills/generate/${persona.id}`)}
              ...
              <div>
                <textarea
                  ...
                  readOnly={!editMode}
                  rows={14}
                  className="w-full text-xs font-mono text-slate-300 rounded-lg p-3 resize-none focus:outline-none transition-all"
                  style={{
                    background: "var(--surface-0)",
                    border: `1px solid ${editMode ? "rgba(34,211,238,0.3)" : "rgba(34,211,238,0.1)"}`,
                  }}
                />
                <div className="flex flex-wrap gap-2 mt-3">
                  {!editMode ? (
                    <button
                      onClick={() => {
                        setEditContent(skillFile.content ?? "");
                        setEditMode(true);
                      }}
                      className="btn-dim text-xs"
                    >
                      Edit
                    </button>
                  ) : (
                  ...
                  {skillFile.status === "REVIEW" && (
                    <button
                      ...
                      disabled={transitioning === "approve"}
                      className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg transition-all disabled:opacity-50"
                      style={{
                  ...
                  {skillFile.status === "APPROVED" && (
                    <button
                      ...
                      disabled={transitioning === "deploy"}
                      className="btn-arc text-xs disabled:opacity-50"
                    >
                      <Rocket size={11} />{" "}
                      {transitioning === "deploy" ? "Deploying…" : "Deploy"}
                    </button>
                  )}
                </div>
              </div>
          ...
          >
            ...
            {agent ? (
              ...
              <p className="text-xs text-slate-600">
                No agent assigned to this persona.
              </p>
            )}
          </div>
        </div>
