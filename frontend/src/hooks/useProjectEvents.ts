...
import { API_URL, getSseToken } from "../api/client";
...
export function useProjectEvents(
  ...
  useEffect(() => {
    ...
    getSseToken(token)
      .then(({ token: sseToken }) => {
        if (cancelled) return;
        const url = `${API_URL}/projects/${projectId}/events?token=${encodeURIComponent(sseToken)}`;
      ...
      .catch(() => {
        // SSE token fetch failed — SSE unavailable, degrade gracefully
        setConnected(false);
      });
